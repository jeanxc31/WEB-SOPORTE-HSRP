...
public function totalCategorias(){
        $sql = "SELECT COUNT(*) as total_categorias FROM categoria WHERE condicion = 1";
        return ejecutarConsulta($sql);
    }

public function totalProductos(){
    $sql = "SELECT COUNT(*) as total_productos FROM articulo WHERE condicion = 1";
    return ejecutarConsulta($sql);
}

public function totalMarcas(){
    $sql = "SELECT COUNT(*) as total_marcas FROM marca WHERE condicion = 1";
    return ejecutarConsulta($sql);
}

public function totalProveedores(){
    $sql = "SELECT COUNT(*) as total_proveedores FROM persona WHERE tipo_persona = 'Proveedor'";
    return ejecutarConsulta($sql);
}
